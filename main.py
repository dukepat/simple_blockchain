...
block_chain.add_transaction(alice_account.create_transaction('bob', 30))
...
block_chain.add_transaction(alice_account.create_transaction('bob', 50, 'This is a top-up on Bob account'))
block_chain.add_transaction(bob_account.create_transaction('dave', 100))
# This should fail after validation check
block_chain.create_new_block()
time.sleep(1)
block_chain.add_transaction(alice_account.create_transaction('bob', 20, 'This is a top-up on Bob account'))
# This should fail after validation check
block_chain.create_new_block()

block_chain.add_transaction(alice_account.create_transaction('bob', 20))
block_chain.add_transaction(bob_account.create_transaction('dave', 30))
block_chain.add_transaction(carol_account.create_transaction('alice', 40))
block_chain.create_new_block()
time.sleep(1)
...
if (validation_result):
    print('Validation successful, any invalid transactions has been returned to the memepool!')
else:
